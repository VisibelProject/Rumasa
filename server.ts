...
CREATE TABLE IF NOT EXISTS coa (
  ...
  name TEXT NOT NULL,
  category TEXT NOT NULL CHECK (category IN ('Income', 'Expense', 'Asset')),
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS journal (
  ...
  category TEXT CHECK (category IN ('Income', 'Expense', 'Asset')),
...
ON CONFLICT DO NOTHING;
  ...
  app.get("/api/branches", async (req, res) => {
    ...
    if (error) {
      console.error("Supabase error in GET /api/branches:", error);
      // Fallback to default branches if table doesn't exist
      return res.json([
        { id: 1, name: "RUMASA", location: "Main" },
        { id: 2, name: "RUMASA HILLSIDE", location: "Hillside" }
      ]);
  ...
  app.post("/api/coa", async (req, res) => {
    ...
    if (error) {
      console.error("Supabase error in POST /api/coa:", error);
  ...
  app.post("/api/journal", async (req, res) => {
    const client = getSupabaseClient();
    if (!client) return res.status(500).json({ error: "Supabase not configured" });
    const { branch_id, date, description, account, debit, credit, category, payment_method } = req.body;
    const { data, error } = await client
      .from("journal")
      .insert([{ branch_id: branch_id || 2, date, description, account, debit, credit, category, payment_method }])
      .select();
  ...
  app.post("/api/assets", async (req, res) => {
    ...
    if (error) {
      console.error("Supabase error in POST /api/assets:", error);
      return res.status(500).json({ error: error.message });
    }
    if (!data || data.length === 0) {
      return res.status(500).json({ error: "Gagal menyimpan data ke database." });
  ...
  app.get("/api/reports/profit-loss", async (req, res) => {
    try {
      ...
      let incomeQuery = client.from("journal").select("credit").eq("category", "Income");
      let expenseQuery = client.from("journal").select("debit").eq("category", "Expense");
      ...
      if ((incomeError && incomeError.code === '42703') || (expenseError && expenseError.code === '42703')) {
        const { data: fIncome } = await client.from("journal").select("credit").eq("category", "Income");
        const { data: fExpense } = await client.from("journal").select("debit").eq("category", "Expense");
        incomeData = fIncome;
  ...
  app.get("/api/menus", async (req, res) => {
    try {
      ...
      // If menu_items fails, try falling back to 'menus' table
      if (menusError) {
        console.error("Supabase error in GET /api/menus (menu_items):", menusError);
        ...
        const { data: fallbackMenus, error: fallbackError } = await fallbackQuery.order("name", { ascending: true });
        
        if (fallbackError) {
          console.error("Supabase error in GET /api/menus (menus fallback):", fallbackError);
      ...
      if (ingError) {
        console.error("Supabase error in GET /api/menus (ingredients):", ingError);
  ...
  app.get("/api/personal-info", async (req, res) => {
    try {
      ...
      if (error) {
        console.error("Supabase error in GET /api/personal-info (personal_info):", error);
        const { data: fallbackData, error: fallbackError } = await client
        ...
        if (fallbackError) {
          console.error("Supabase error in GET /api/personal-info (personal_information fallback):", fallbackError);
          return res.json([]); // Return empty array if both fail
        }
        data = fallbackData;
      }
